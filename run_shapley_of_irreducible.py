...
def get_random_source_sets(players:list[Node]):
    ...
    source_a_set.add(random_player)
    source_b_set.add(random_player)
...
def run():
    ...
    while limiter < limit:
        ...
        coalitions_reduced_graph = coop_reduced_graph.get_player_coalition_values(source_a_set, source_b_set)
...
Sum of cost allocation and grand coalition cost: {sum(shapley_value)} != {list(coalitions_original_graph.values())[-1]}\n\n"""
                with open('contradictions_shapley_value_both_sources.txt', 'a') as file:
                    file.write(data)
